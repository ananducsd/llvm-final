  ...
  EdgeFact * runFlowFn(EdgeFact *in, Node<EdgeFact> *node) {
    switch(node->I->getOpcode()) {
      ...
      case Instruction::PHI: {
        //todo
        PHINode *PN = dyn_cast<PHINode>(node->I);
        std::set<Constant *> phiConstants; 
        Constant *C2;
        for (unsigned i = 0, e = PN->getNumIncomingValues(); i != e; ++i) {
                ...
                if (isa<UndefValue>(Incoming))
                    continue;
                C2 = dyn_cast<Constant>(Incoming);
                if(!C2) break;
                phiConstants.insert(C2);                  
        }
        errs() << "phi set size = " << phiConstants.size() << "\n";
        if(phiConstants.size() >= 1) {
            /*set<Constant *>::iterator i = phiConstants.begin();
            errs() << "C2 = " << **i << "\n";
            errs() << "Instr name = " << node->I->getName() << "\n";
          
            set<Value *> res; 
            res.insert(*i);
            Fact * newFact = new Fact(res);
            in->kill(node->I);
            in->addFact(node->I, newFact );*/
        }
