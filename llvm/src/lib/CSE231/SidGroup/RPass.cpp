...
namespace {
  ...
  struct RangePass : public ModulePass {
    ...
    bool runOnModule(Module &M){
      ...
      r_workListObj* wl = new r_workListObj(10010);
      wl->init(M);
      wl->run();
      //print results...
      //M.dump();
      ...
      for(map<int, r_BBNode*>::iterator it = wl->bbMap.begin(); it != wl->bbMap.end(); it++) {
      
        //TODO : track all allocations!
        r_BBNode * BBN = it->second;
        //For each node
        for(unsigned int i=0; i<BBN->nodes.size(); i++){
          r_Node * N = BBN->nodes[i];
          //llvm::Instruction::GetElementPtr
          if(N->oI->getOpcode() == Instruction::GetElementPtr && isa<AllocaInst>(N->oI->getOperand(0))){
            //get the first operand (the array)
            AllocaInst * op0 = dyn_cast<AllocaInst>(N->oI->getOperand(0));
            //get the third operand (the array index)
            Value * op2 = N->oI->getOperand(2);
